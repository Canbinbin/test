//自动补全
;;var AutoCompleteSelector = (function(window, $, Mustache, undefined){
	...
	$.extend(prototypeFn, {
		...
		destroy: function(){
			...
			that._$container.off(".autocomplete");//移除事件
			$(document).off("click.autocomplete.others", that._clickOthersEventCallback);
		...
		_initComponent: function(){
			...
			if(!$target || !$target.length){
				throw new Error("初始化时，请传入$target参数");
			}else{
				cloneNodes = [
					$target
						.clone()
						...
						.removeClass("required notEqualTo"),
					$target
						.clone()
						.prop("readonly", true)
		...
		_bindEventListener: function(){
			...
			that._$container
				.on("click.autocomplete.target", targetInput, function(e){
					eventCallbacks.clickTargetInput.call(that, e, $(this));	
				})
				.on("click.autocomplete.dropdownmenuitem", dropdownMenuItem, function(e){
					eventCallbacks.clickDropdownMenuItem.call(that, e, $(this));
				})
				.on("keydown.autocomplete.inputbox", inputBox, function(e){
					eventCallbacks.keydownInputbox.call(that, e, $(this));
				});
				/*.on("focusout.autocomplete.inputbox", inputBox, function(e){
					eventCallbacks.focusoutInputBox.call(that);
				})*/
			that._$container.find(inputBox).on("input.autocomplete.inputbox propertychange.autocomplete.inputbox", function(e){
		...
		_showInputBox: function(){
			...
			that._clearHideInputBoxTimer();
			$inputBox.show().focus();
			that._clickOthersEventCallback = function(e){
				that._eventCallbacks.clickOthers.call(that, e, $(e.target));
			};
			$(document).on("click.autocomplete.others", that._clickOthersEventCallback);
		...
		_eventCallbacks: {
			...
			clickDropdownMenuItem: function(e, $target){
				...
				if(!$target.hasClass(DISABLED_CLASS_NAME)){
					that._$dropdownMenu.find("." + ACTIVE_CLASS_NAME).removeClass(ACTIVE_CLASS_NAME);
