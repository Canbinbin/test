(function(window, $, undefined) {
    var Main = {
        ...
        _bindEvent: function() {
            var that = this;
            that.$submitList.on("click", function() {
                that._initValidate(1);
            });
        ...
        _initValidate: function(flag) {
            ...
            that.$myform.validate({
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "/admin/news/firstcolumn/save", //保存数据
                        dataType: "json",
                        type:   "POST",
                        ...
                        success: function(data) { //保存成功后返回的数据
                            layer.msg(data.msg, {
                                ...
                                if (flag == 1) {
                                    window.location.href = "/admin/news/firstcolumn";
                                } else if (flag == 2) {
                                    // window.location.reload(true);
                                	window.location.href = "/admin/news/firstcolumn/add";
                                }
                            })
            ...
            if (value) {
                if (~$.inArray(value.split("\.").pop(), imageType)) {
                    $.ajaxFileUpload({
                        ...
                        secureuri: false,
                        fileElementId: "file",
                        url: "/admin/news/firstcolumn/upload_image?date=" + new Date().getTime(),
