	...
	$.fn.zyComment = function(options,param){
		...
		return this.each(function(){
			...
			this.createAgoCommentHtml = function(){
				...
				html += '	<div class="text" style="font-size:2rem;padding-bottom:10px;border-bottom: 1px solid #DFDFDF;"> 讨论区 </div>';
				html += '</div>';
				$(self).append(html);
				...
				$.each(para.agoComment, function(k, v){
					...
					item += '	<a class="avatar">';
					item += '		<img src="/static/image/home/inner/foot.png">';
					item += '	</a>';
			...
			this.replyClickEvent = function(){
				...
				$(self).find(".actions .reply").live("click", function(){
					...
					$("#publicComment").live("click",function(){
						...
						else if(content.length>254){
							layer.msg("字数过多，请限制在254个字以内！");
							return;
						}
						var result = {
							    "sortID":sortID,
